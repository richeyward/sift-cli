...
const siftowner = 'richeyward'
const siftrepo = 'sift-saltstack'
...
function setupSalt() {
  if (cli['--dev'] === false) {
    return co.execute(function * () {
      ...
      const saltstackBaseUrl = 'http://repo.saltstack.com/apt/ubuntu/18.04/amd64/archive/2019.2.0'
      const aptDebString = `deb ${saltstackBaseUrl} bionic main`
      const aptGpgUrl = `${saltstackBaseUrl}/SALTSTACK-GPG-KEY.pub`
      ...
      if (aptExists === true && saltVersionOk === false) {
        ...
        yield fs.writeFileAsync(aptSourceList, aptDebString)
        yield child_process.execAsync(`wget -O - ${aptGpgUrl} | apt-key add -`)
...
function downloadReleaseFile (version, filename) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = request.get(`https://github.com/${siftowner}/${siftrepo}/releases/download/${version}/${filename}`)
    req.on('error', (err) => {
...
function downloadRelease (version) {
  ...
  return new Promise((resolve, reject) => {
    const output = fs.createWriteStream(filepath)
    const req = request.get(`https://github.com/${siftowner}/${siftrepo}/archive/${version}.tar.gz`)
    req.on('error', (err) => {
